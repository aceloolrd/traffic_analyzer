...
class Tracker:
    ...
    def __init__(
        ...
        self.kf = kalman_filter.ExtendedKalmanFilter(dt=1.0)
        # self.kf = kalman_filter.ExtendedKalmanFilterAccel(dt=1.0)
        self.tracks = []
        self.del_tracks_ids = []
        self._next_id = 1
    ...
    def update(self, detections, today=None):
        ...
        # matches — пары (номер_трека, номер_детекции), где успешно сопоставлено
        # unmatched_tracks — индексы треков, которые не нашли себе подходящей детекции
        # unmatched_detections — индексы новых детекций, которые не были сопоставлены ни с одним треком
        ...
        # опеределяем какие треки удалить а какие оставить
        # Удаляем треки, которые не были подтверждены в течение n_init кадров
        new_tracks = []
        self.del_tracks_ids = []
        ...
        for track in self.tracks:
            if not track.is_confirmed():
                continue
            # список признаков треков, которые будут использоваться для обновления метрики
            features += track.features
            # id этих треков
            targets += [track.track_id for _ in track.features]
            # оставляем только последний вектор признаков трека
            track.features = [track.features[-1]]
        # Обновляем метрику, используя векторы признаков и id треков
    ...
    def _match(self, detections):
        ...
        ############################################################################################
        # print("[MATCH_A]", matches_a, " | [MATCH_B]", matches_b)
        # print("[UNMATCHED_TRACKS]", unmatched_tracks, " [UNMATCHED_DETS]", unmatched_detections)
        ############################################################################################
