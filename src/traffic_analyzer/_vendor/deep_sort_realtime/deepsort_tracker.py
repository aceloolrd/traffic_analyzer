...
class DeepSort(object):
    ...
    def update_tracks(self, raw_detections, embeds=None, frame=None, today=None, others=None, instance_masks=None):
        ...
        if len(raw_detections) > 0: 
            if not self.polygon:
                ...
                # если не получены emb со стороны, то делаем сами
                if embeds is None:
                    embeds = self.generate_embeds(frame, raw_detections, instance_masks=instance_masks)

                # Формируются объекты детекции DeepSort, которые включают координаты бокса, уровень уверенности и вектор признаков appearance
    ...
    def generate_embeds(self, frame, raw_dets, instance_masks=None):
        ...
        if cropped_inst_masks is not None:
            masked_crops = []
            # заливаем фон на кропе чтобы учитывать внешность только в области маски
            # и не учитывать фон, который может быть разным в разных кадрах
            ...
            # если маски нет, то анализируем целиком с фоном
    ...
    # создаем список детекций DeepSort из сырых детекций и векторов признаков
    ...
    # вырезает bbox из кадра и возвращает список вырезанных изображений
    # если instance_masks не None, то вырезает только область маски
    # и возвращает список вырезанных изображений и масок
    # если instance_masks None, то возвращает только список вырезанных изображений
    @staticmethod
